...
public class PassengerOrderController extends BaseController {
    ...
    public Result<?> createOrder(@RequestBody Map<String, Object> params, HttpServletRequest request) {
        ...
        List<DriverInfo> onlineDrivers = driverInfoMapper.selectList(
                new QueryWrapper<DriverInfo>()
                        .eq("online_status", Constants.ONLINE)
        ...
        if (!onlineDrivers.isEmpty()) {
            ...
            order.setStatus(Constants.ORDER_DISPATCHED); // 已派单
        } else {
            order.setStatus(Constants.ORDER_DISPATCHING); // 待派单
        }
    ...
    public Result<?> cancelOrder(@PathVariable Long orderId,
        ...
        order.setStatus(Constants.ORDER_CANCELLED_PASSENGER); // 乘客取消
